...
const modal = (children, { className, title, desc, icon, offset, margin, closer = 'both', placement = 'center' }) => {
    ...
    const ModalComponent = () => {
        return createPortal(_jsx(Portal, { children: _jsxs("div", { className: cn('fixed flex flex-col w-fit border-2 border-card bg-card rounded-lg gap-2 p-4 z-100 pointer-events-auto', opened ? 'opacity-100' : 'opacity-0', className), style: positioning, ref: modal, inert: !opened, "aria-hidden": !opened, slot: 'card', children: [_jsx(Header, { icon: icon, title: title, desc: desc }), children] }) }), layer);
    };
